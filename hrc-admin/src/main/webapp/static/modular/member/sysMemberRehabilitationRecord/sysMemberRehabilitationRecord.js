...
MemberRehabilitationRecord.openAddMemberRehabilitationRecord = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/sysMemberRehabilitationRecord/memberRehabilitationRecord_add'
    });
...
MemberRehabilitationRecord.openMemberRehabilitationRecordDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/sysMemberRehabilitationRecord/sysMemberRehabilitationRecord_detail/' + MemberRehabilitationRecord.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MemberRehabilitationRecord.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/sysMemberRehabilitationRecord/delete", function (data) {
            Feng.success("删除成功!");
            MemberRehabilitationRecord.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("sysMemberRehabilitationRecordId",this.seItem.id);
        ajax.start();
    }
};
...
$(function () {
    ...
    var table = new BSTable(MemberRehabilitationRecord.id, "/sysMemberRehabilitationRecord/list", defaultColunms);
