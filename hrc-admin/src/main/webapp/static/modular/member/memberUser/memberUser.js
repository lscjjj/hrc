...
MemberUser.openAddMemberUser = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/memberUser/memberUser_add'
    });
...
MemberUser.openMemberUserDetail = function () {
    // if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/memberUser/memberUser_update'
        });
        this.layerIndex = index;
    // }
};
...
MemberUser.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/memberUser/delete", function (data) {
            Feng.success("删除成功!");
            MemberUser.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("memberUserId",this.seItem.id);
        ajax.start();
    }
};
...
$(function () {
    ...
    var table = new BSTable(MemberUser.id, "/memberUser/list", defaultColunms);
