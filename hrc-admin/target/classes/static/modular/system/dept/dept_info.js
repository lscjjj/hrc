 ...
 * @returns
 */
DeptInfoDlg.onClickRegion = function(e, treeId, treeNode) {
    $("#pRegionName").attr("value", DeptInfoDlg.zTreeInstanceRegion.getSelectedVal());
    $("#region").attr("value", treeNode.id);
}
 ...
 * @returns
 */
DeptInfoDlg.showRegionSelectTree = function() {
    var pName = $("#pRegionName");
    var pNameOffset = $("#pRegionName").offset();
    $("#parentRegionMenu").css({
        left : pNameOffset.left + "px",
        top : pNameOffset.top + pName.outerHeight() + "px"
    }).slideDown("fast");
 ...
 * 隐藏城市选择的树
 */
DeptInfoDlg.hideRegionSelectTree = function() {
    $("#parentRegionMenu").fadeOut("fast");
    $("body").unbind("mousedown", onRegionBodyDown);// mousedown当鼠标按下就可以触发，不用弹起
}
...
DeptInfoDlg.collectData = function() {
    this.set('id').set('simplename').set('fullname').set('tips').set('num').set('pid').set('region')
        .set('isStore').set('address').set('appointmentTelephone').set('principal').set('principalPhone').set('priceStandard');
}
...
function onRegionBodyDown(event) {
    if (!(event.target.id == "menuBtn" || event.target.id == "parentRegionMenu" || $(
            event.target).parents("#parentRegionMenu").length > 0)) {
        DeptInfoDlg.hideRegionSelectTree();
    }
}
...
$(function() {
    ...
    var ztreeRegion = new $ZTree("parentRegionMenuTree", "/dept/treeRegion");
    ztreeRegion.bindOnClick(DeptInfoDlg.onClickRegion);
    ztreeRegion.init();
    DeptInfoDlg.zTreeInstanceRegion = ztreeRegion;

    $("#isStore").val($("#isStoreValue").val());
    $("#priceStandard").val($("#priceStandardValue").val());
});
