 ...
 * 初始化表格的列
 */
OldHistory.initColumn = function () {
    return [
 ...
 * 检查是否选中
 */
OldHistory.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        OldHistory.seItem = selected[0];
        return true;
    }
};
...
$(function () {
    var defaultColunms = OldHistory.initColumn();
    var table = new BSTable(OldHistory.id, "/oldMember/update_history_list/"+localStorage.getItem("uid"), defaultColunms);
    table.setPaginationType("client");
    OldHistory.table = table.init();
