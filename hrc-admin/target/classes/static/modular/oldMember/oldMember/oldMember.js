...
OldMember.check = function () {
    ...
    if (selected.length == 0) {
        ...
        // localStorage.clear();
        OldMember.seItem = selected[selected.length-1];
 ...
 * 点击添加会员
 */
OldMember.openAddMember = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/oldMember/member_add'
    });
 ...
 * 点击跳转调理方案管理
 */
OldMember.openHealthRecordList = function () {
    OldMember.strx = '';
    var index = layer.open({
        type: 2,
        title: '添加方案',
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/memberHealthRecord/memberHealthRecord_add#'+OldMember.strx
 ...
 * 点击跳转康复项目
 */
OldMember.openTreatmentList = function () {
 ...
 * 点击跳转会员充值
 */
OldMember.openMemberShipChargeLogList = function () {
    var index = layer.open({
        type: 2,
        title: '会员卡充值',
 ...
 * 点击跳转会员结算
 */
OldMember.openMemberSettlementList = function () {
    $.ajax({
 ...
 * 打开查看会员详情
 */
OldMember.openMemberDetail = function (e) {
    console.log(this);
    if (this.check()) {
        var index = layer.open({
            type: 2,
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/oldMember/member_update/' + OldMember.seItem.id
        });
        this.layerIndex = index;
        layer.full(index);
    }
 ...
 * 删除会员
 */
OldMember.delete = function () {
    if (this.check()) {
        var item = this;
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/oldMember/delete", function (data) {
                if (data.code == 200) {
                    Feng.success("删除成功!");
                    OldMember.table.refresh();
                } else {
...
$(function () {
    var defaultColunms = OldMember.initColumn();
    var table = new BSTable(OldMember.id, "/oldMember/list", defaultColunms);
    table.setPaginationType("client");
    OldMember.table = table.init();
...
function more(id) {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/oldMember/member_update/' + id
    });
    this.layerIndex = index;
    layer.full(index);
}
...
function history(id) {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/oldMember/history_list/' + id
    });
    this.layerIndex = index;
    layer.full(index);
}
