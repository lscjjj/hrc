...
function check_initx(str, str_id) {
    if ($(window.parent.document.getElementById('tcmSurveyInfo')).val() != undefined) {
        let count_val = $(window.parent.document.getElementById('tcmSurveyInfo')).val().split('。');
...
function check_if(ting) {
    let data_val = $($(ting).parent()[0]).attr('data-val');
    let check_name = data_val + "：" +$(ting).parent()[0].innerText ;
    let suerver_sf = $(window.parent.document.getElementById('tcmSurveyInfo')).val();
    let arr_sf = [];
    if ($(ting).attr('checked') == "checked") {
        $(ting).attr('checked', false);
        var survey_info = $(window.parent.document.getElementById('tcmSurveyInfo')).val();
        survey_info = survey_info.split(',');
        let num = survey_info.indexOf(check_name);
        survey_info.splice(num, 1);
        $(window.parent.document.getElementById('tcmSurveyInfo')).val(survey_info.join());
    } else {
        ...
        $(window.parent.document.getElementById('tcmSurveyInfo')).val(arr_sf.join('。'));
 ...
 * 将所有数据放入数组
 */
function Allputarr(data) {
    //数据数组创建 103个数组
    var sum = data[data.length - 1].location;
    var arr = new Array();
    var arrid = new Array();
    var module = new Array();
    //给定每个列的长度 如第一列10个 第二列9个
    var num = [10, 9, 7, 10, 3, 6, 5, 10, 8, 3, 8, 8,19,3,3,5,19];
    for (var i = 0; i < data.length; i++) {
        if (!arr[i] && arr.length <= 104) {
            arr[i] = [];
            arrid[i] = [];
            module[i] = [];
        }

        arr[data[i].location - 1].push(data[i].name);
        arrid[data[i].location - 1].push(data[i].id);
        module[data[i].location - 1].push(data[i].module);
    }
    AllDomeset(arr, num, arrid, module);
}
 ...
 * 把处理好的数组arr进行dom操作
 */
function AllDomeset(arr, num, arrid, module) {
    //2个循环出数组的值
    var vc = '';
    for (var i = 0; i < arr.length; i++) {
        ...
        if(i<87){
            ...
            for (var s = 0; s < arr[i].length; s++) {
                if(arr[i][s].indexOf("：") != "-1")
                {
                    var sh = arr[i][s].split('：');
                    arr[i][s] = '<br>'+sh[0] + '：<br>' + sh[1];
                }
                vc = '<p id="a_' + arrid[i][s] + '" data-val="' + module[i][s] + '" >' + arr[i][s] + '<input type="checkbox" onclick="check_if(this)"> </p>';
                $(trid).append(vc);
                var val_id = '#a_' + arrid[i][s];
                var val = module[i][s] + "：" + arr[i][s];
                check_initx(val,val_id)
            }
        }else {
            if($(obj.trids).children().length >= num[obj.num]){
                obj.num += 1;
                obj.trids = "#fast" + obj.num;
            }
            var trids = obj.trids;
            for (var s = 0; s < arr[i].length; s++) {
                vc = '<span id="a_' + arrid[i][s] + '">' + arr[i][s] + '<input type="checkbox" onclick="check_if(this)"> </span>';
                $(trids).append(vc);
            }
        }
        ...
        //判断一列多少条数据
        if($(obj.cd).children().length > num[obj.num] && i<87)
        {
            //填补空白
            var g = 10 - num[obj.num]
            for(var n=0;n<g;n++)
            {
                var trs = "<td></td>";
                $(obj.cd).append(trs);
